  ...
  <section class="flex h-full min-h-0 flex-col gap-4 p-4">
    <div class="flex flex-col gap-2">
      ...
      <select
        ...
        class="h-8 w-[300px] rounded-[var(--radius-control)] border border-transparent bg-muted px-2 text-[12px] outline-none focus:border-primary focus:bg-card"
      >
    ...
    <div class="flex items-center gap-3">
      ...
      <button type="button" class="h-8 rounded-[var(--radius-control)] border border-input px-2.5 text-[12px] hover:bg-muted" @click="connectDevice">
        Connect
      </button>
      <span class="font-mono text-[10.5px] text-muted-foreground">USB printer class devices only</span>
    </div>

    <div class="w-fit rounded-[var(--radius-control)] border border-input p-3">
