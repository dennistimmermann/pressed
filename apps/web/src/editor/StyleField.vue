<!--
  One field of the STYLE grid = one declaration (DESIGN "Field anatomy"): the shared 25px field
  (`@/ui` · `Field`) holding the value and, on the right, the unit as a chip-select. Unset shows `–`; a
  keyword (`auto`) is kept as typed and greys the chip; ↑↓ steps, ⇧ ×10. Empty removes the
  declaration. A value that is in force but not written here (from a shorthand, from the base
  sheet) is drawn in `--inherited-foreground`; typing over it writes this field's own declaration.
  `×` — on hover or focus, only while there *is* an own declaration — removes it again.
...
/** With a choice the chip is a select; otherwise it is the one unit, as text. */
const unitSelect = computed(() => !!props.units && props.units.length > 1 && !props.disabled)
const unitText = computed(() =>
  !props.units || unitSelect.value ? undefined : props.units.length > 1 ? unit.value : props.units[0],
)
...
<template>
  <Labeled
    :label="label" :class="{ bad, driven: disabled }"
    :title="disabled ? `follows ${from}` : !own && from ? `from ${from}` : !own && inherited ? `${prop} — in effect but not set in this rule` : prop"
  >
    <Field
      :class="{ sel: kind === 'select' }" :icon="icon" :unit="unitText" :unit-off="keyword || disabled"
      :unset="own && !disabled" @unset="emit('set', null)"
    >
      ...
      <!-- The unset row is `– (absolute)`: the dash says "not set in this rule", the parens say
           what applies anyway (another rule, the base sheet, or the browser default — for most
           properties that is not CSS inheritance, so we never call it that). -->
      <select
        v-if="kind === 'select'" class="v" :class="{ inh: !own }" :value="value ?? ''" :aria-label="prop"
        :title="own ? undefined : inherited ? 'not set in this rule — in effect from another rule, the base stylesheet or the browser default' : undefined"
      ...
      <!-- The chip stays one keyboard stop: the select wears it. -->
      <template v-if="unitSelect" #unit>
        <select :value="unit" :aria-label="`${prop} unit`" @change="pickUnit">
