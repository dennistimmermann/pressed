...
const groups = computed(() =>
  [facets.value, sizes.value].map((group) => group.map((f) => ({ ...f, on: filter.value === f.key }))),
)
...
const note = computed(() =>
  skipped.value.length ? { lead: 'skipped (only .vue imports):', detail: skipped.value.join(', ') } : null,
)
...
<template>
  <Library
    ...
    :open="open"
    :title="pick ? 'Choose a template' : 'Templates'"
    :label="pick ? 'Choose a template' : 'Manage templates'"
  ...
  >
    ...
    <template #default="{ dialog }">
      <section v-for="group in sections" :key="group.kind" class="mb-3 last:mb-0">
      ...
      <!-- Teleported *into* the dialog: a modal `<dialog>` owns the top layer and makes the rest
