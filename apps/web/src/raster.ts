 ...
 */
export async function rasterize(label: RenderedLabel, size: Meta['size'], profile: PrinterProfile): Promise<ImageData> {
  ...
  const doc = new DOMParser().parseFromString(labelDocument(label, size), 'text/html')
 ...
 */
export async function rasterDataUrl(label: RenderedLabel, size: Meta['size'], profile: PrinterProfile): Promise<string> {
  const image = await rasterize({ html: stripDataLoc(label.html), css: label.css }, size, profile)
