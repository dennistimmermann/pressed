...
/** `on` = written in this rule, `muted` = only in the base stylesheet. */
...
<template>
  <div class="pane">
    ...
    <div v-for="b in [SHORTHAND.padding, SHORTHAND.margin]" :key="b.key" class="f">
      <div class="hdr">
      ...
      <div v-else class="cells">
        <StyleField
          v-for="c in cells(b)" :key="c.prop" :prop="c.prop" :icon="c.icon" :value="c.value"
    ...
    <div v-for="b in [SHORTHAND.radius]" :key="b.key" class="f">
      <div class="hdr">
        <span class="k">{{ b.label }}</span>
        <span class="tog" :class="{ off: raw(b) }" :title="raw(b) ? 'edit as text' : undefined">
          <StyleSeg :id="`css-msg-${b.prop}-arity`" :choices="aritySeg(b)" @pick="setArity(b, Number($event))" />
...
/* padding · margin · radius: the label with the arity toggle on its right, over the 2×2 of
   sides — column 1 is T/B (TL/BL), column 2 is L/R (TR/BR). */
.hdr { display: flex; align-items: center; gap: 6px; min-width: 0; }
.hdr .k { flex: 1; min-width: 0; }
.tog { flex: none; }
