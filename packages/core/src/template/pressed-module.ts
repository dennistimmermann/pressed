...
/**
 * The `pressed` module template code imports from. It exists twice on purpose:
 * as a real module (so the library `.vue` files compile under Vite/vue-tsc, via the
 * `pressed` alias) and as an entry in `__modules__` at runtime — same file, same exports.
